...
⚠️ **A third hazard: `mwapi:searchRelation`'s jointly-bound property+item pair, when used inside `FILTER NOT EXISTS`/`MINUS`, or inside a `UNION` over both subject/object direction (for a symmetric relation like spouse), triggers the unbound-predicate-object scan even fully subquery-isolated per CQ9's fix** (confirmed: both forms hit a 65 GB allocation attempt / 30s timeout). The problem is specifically the joint two-variable binding, not negation or `UNION` themselves, and not the presence/absence of subquery isolation — **a single SERVICE-bound variable, resolved independently of its counterpart, works fine in both positions once isolated.** Fix, confirmed working: resolve the *property* via plain `mwapi:search`/`mwapi:bind` (not `searchRelation`), resolve the *item* via a direct `rdfs:label` exact-match (the CQ12 pattern) instead of jointly, and isolate the SERVICE+join in its own subquery (the CQ9 pattern) — do this once for `FILTER NOT EXISTS`/`MINUS`, and once *per branch* (each with its own output variable) for a direction-`UNION`. See `exemplars.ttl`'s CQ10 (negation) and CQ11 (symmetric relation) for the working single-query SERVICE variants.
